...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      className={cn(
        "web:inline-flex h-10 native:h-12 items-center justify-center rounded-md bg-foreground p-1 native:px-1.5",
        className
      )}
      {...props}
    />
...
function TabsTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        "text-sm native:text-base font-medium text-muted web:transition-all",
        value === props.value && "text-foreground font-bold"
      )}
    >
